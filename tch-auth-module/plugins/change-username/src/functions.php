...
function ajax_handler() {
    ...
    if( ! validate_username( $new_username ) ) {
        $response['message'] = __( 'Это имя пользователя недействительно, поскольку в нем используются недопустимые символы. Пожалуйста введите действительное имя пользователя.' );
        wp_send_json($response);
    }
    ...
    if ( in_array( $new_username, $illegal_user_logins ) ) {
        $response['message'] =  __( 'Извините, это имя пользователя не разрешено.' );
        wp_send_json($response);
    }

    // check if new username is in use already
    if( username_exists( $new_username ) ) {
        $response['message'] = sprintf( '<strong>%s</strong> уже используется.', $new_username );
        wp_send_json($response);
    }
    ...
    $response['message'] = sprintf( 'Имя пользователя успешно изменен на <strong>%s</strong>.', $new_username );
    wp_send_json($response);
}
